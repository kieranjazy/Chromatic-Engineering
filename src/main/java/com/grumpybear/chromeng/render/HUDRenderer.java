...
public class HUDRenderer{
   ...
   public static void renderMode(RenderGameOverlayEvent.Post event) {
      ...
      int sx = res.getScaledWidth() / 2 - 17; //196.5 - 223
      int sy = res.getScaledHeight() / 2 + 2; //122 - 129.5
      ...
      if (player.getHeldItemMainhand().getItem() instanceof IModeItem) {
         ...
         font.drawString("Mode: " + modeItem.getCurrentModeString(player.getHeldItemMainhand()), sx + 110, sy + 109, modeItem instanceof IChargeableSingle ? EnumColourUtil.colourToRGB(((IChargeableSingle) modeItem).getColourType()) : 0xFF4500);
      }
      ...
      if (player.getHeldItemMainhand().getItem() instanceof ItemExtensionConduit) {
         ...
         if (item.getMode(player.getHeldItemMainhand()) == 2 && BlockPosUtil.nbtHasPos(tag, 0) && BlockPosUtil.nbtHasPos(tag, 1)) {
            ...
            int required = item.getFinalBlockList(BlockPosUtil.getFromNBT(tag, 0), BlockPosUtil.getFromNBT(tag, 1)).size() - 2;
            font.drawString((total <= required ? String.valueOf(total) : String.valueOf(required)) + "/" + String.valueOf(required), sx + 200, sy + 109, EnumColourUtil.colourToRGB(EnumColour.YELLOW));
            GlStateManager.pushMatrix();
            GlStateManager.enableLighting();
            GlStateManager.enableTexture2D();
            mc.getRenderItem().renderItemIntoGUI(new ItemStack(Item.getItemFromBlock(blockstate.getBlock())), sx + 240, sy + 105);
